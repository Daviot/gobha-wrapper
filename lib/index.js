...
function wrapper(opts, final) {
    ...
    let ms = Metalsmith(dir)
        ...
        .use(
            assets({
                // copy assets into the destionation path
        ...
        .use(
            assets({
                // copy material design components into the destionation path
        ...
        .use(
            fileToPath({
                // create permalinks and modify the file extension based on the settings in the file "ext: html"
                logging: logging
            })
        )
        ...
        .use(
            navigation({
                // create navigation out of the setting in the file "nav: header"
                logging: logging
            })
        )
        .use(
            partials({
                // convert partials and handlebar code in the page files itself
                partials: partialDir,
        ...
        .use(
            assets({
                // copy php scripts into the destionation path
        ...
        .use(
            uglifyjs({
                src: ['**/*.js', '!**/*.min.js'],
                target: function(inFile) {
                    return inFile.replace('.js', '') + '.min.js';
                },
                deleteSources: true,
                uglifyOptions: {
                    mangle: true,
        ...
        .use(inlineSource({ pattern: ['*.htm', '*.html', 'de/*.php', 'de/**/*.php'] }));
    if (minify) {
        ms = ms.use(htmlMinifier('*.html'));
    }
